       ...
       cs++ )
  {
	...
	restore_record(table, s->default_values);
        ...
        else
        {
          ...
          table->field[3]->store(Show::Yesno::value(tmp_cl->default_flag()), scs);
        }
        table->field[4]->store(Show::Yesno::value(tmp_cl->compiled_flag()), scs);
...
int fill_schema_coll_charset_app(THD *thd, TABLE_LIST *tables, COND *cond)
{
       ...
       cs++ )
  {
	...
	continue;
      ...
      table->field[4]->store(Show::Yesno::value(tmp_cl->default_flag()), scs);
      if (schema_table_store_record(thd, table))
        return 1;
    }
  }
...
ST_FIELD_INFO collation_fields_info[]=
{
  ...
  Column("IS_DEFAULT",                   Yesno(),      NULLABLE, "Default"),
...
ST_FIELD_INFO coll_charset_app_fields_info[]=
{
  ...
  Column("IS_DEFAULT",         Yesno(),  NOT_NULL),
  CEnd()
};
